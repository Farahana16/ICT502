...
public class ViewOrderDAO {
	...
	public static List<View> getAllViewByCustID(int custID){
		...
		try {
			...
			PreparedStatement ps = con.prepareStatement("SELECT p.*, o.* FROM product p join orderdetails od on od.productid = p.productid join orderr o on o.orderid = od.orderid where o.custid =?");
			ps.setInt(1, custID);
